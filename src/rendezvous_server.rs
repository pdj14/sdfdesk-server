    ...
    pub async fn start(port: i32, serial: i32, key: &str, rmem: usize) -> ResultType<()> {
        ...
        tokio::spawn(async move {
            let app = axum::Router::new()
                .route("/api/peers", axum::routing::get(move || async move {
                    let peers = peer_map.all_peers().await;
                    let json_peers: Vec<serde_json::Value> = peers.into_iter().map(|(id, ip, online)| {
                        serde_json::json!({
                            "id": id,
                            "ip": ip,
                            "online": online
