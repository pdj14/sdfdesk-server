...
pub const REG_TIMEOUT: i32 = 30_000;  // Same as rendezvous_server
...
impl PeerMap {
    ...
    /// Returns all registered peers with ID, IP, and online status
    pub async fn all_peers(&self) -> Vec<(String, String, bool)> {
        let map = self.map.read().await;
        let mut result = Vec::new();
        for (id, peer) in map.iter() {
            let peer = peer.read().await;
            let online = peer.last_reg_time.elapsed().as_millis() < (REG_TIMEOUT as u128);
            result.push((id.clone(), peer.info.ip.clone(), online));
        }
        result
    }
}
